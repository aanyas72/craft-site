  ...
  return (
    <header className="w-full bg-[#ffffff] text-[#2d1c10] sticky top-0 z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between pb-3 pt-3">
        {/* Logo */}
        <div
          className="font-bold text-2xl tracking-tight hover:cursor-pointer"
          onClick={() => router.push("/")}
        >
          Logo
        </div>
        ...
        <nav className="flex items-center gap-4">
          {loading ? (
            ...
            <button
              aria-label="Profile"
              className="p-2 hover:bg-[#ece7db] rounded-full transition"
      ...
      <div className="w-full border border-[#cfc8c2] bg-[#ffffff]">
        <div className="max-w-7xl mx-auto flex justify-center gap-8 pb-3 pt-3">
          <button
            className="font-medium text-[#2d1c10] hover:text-[#bfa77a] transition"
            onClick={() => router.push("/video-discovery")}
          >
            Video Discovery
          </button>
          <button
